...
from app.serializers import ProductListingSerializer, MyUserListingSerializer, PasswordResetSerializer, \
    AgentAllListingSerializer, QaUserListingSerializer
...
class QaAgentAdd(APIView):
    ...
    def post(self, request):
        try:
            ...
            try:
                MyUser.objects.get(Q(email=email) & Q(is_active=True))
            ...
            user_obj = MyUser.objects.create_user(agent_name=agent_name.title(), email=email, password=random_password,
                                                  mobile=mobile)
            user_obj.is_qa_agent = True
            user_obj.is_active = True
            user_obj.signup_date = date.today()
            user_obj.save()
            return Response({"data": {"is_qa_agent_added": True, "password": random_password,
                                      'message': 'QA Agent Created successfully'}}, status=status.HTTP_201_CREATED)
        except ErrorMessage as e:
            return Response({"data": {"is_qa_agent_added": False, 'message': e.message}}, status=status.HTTP_200_OK)
...
class QaAgentPasswordReset(APIView):
    permission_classes = (IsAuthenticated, AdminValidation)
        ...
        try:
            ...
            try:
                user_obj = MyUser.objects.get(Q(agent_id=agent_id) & Q(is_active=True)
                                              & Q(is_superuser=False) & Q(is_qa_agent=True))
            except MyUser.DoesNotExist:
...
class QaAgentlisting(generics.ListAPIView):
    permission_classes = (IsAuthenticated, AdminValidation)
    ...
    def get_queryset(self):
        ...
        queryset = MyUser.objects.filter(Q(is_qa_agent=True) & Q(is_active=True) & Q(agentsearch(search))).order_by(order)
        count = len(queryset)
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if count:
                serializer = QaUserListingSerializer(data_obj, many=True)
...
class QaAgentEdit(APIView):
    ...
    def post(self, request):
        try:
            ...
            try:
                agent_obj = MyUser.objects.get(Q(agent_id=agent_id) & Q(is_qa_agent=True) & Q(is_active=True))
            except MyUser.DoesNotExist:
            ...
            if not email_check:
                raise ErrorMessage("Email is not valid")
            try:
                email_check_obj = MyUser.objects.get(Q(email=email) & Q(is_active=True))
            ...
            agent_obj.save()
            return Response({"data": {"is_qa_agent_updated": True,
                                      'message': 'QA Details Updated successfully'}}, status=status.HTTP_200_OK)
        except ErrorMessage as e:
...
class QaAgentDelete(APIView):
    ...
    def delete(self, request):
        try:
            ...
            try:
                user_obj = MyUser.objects.get(Q(is_active=True) & Q(agent_id=agent_id) & Q(is_active=True) & Q(is_qa_agent=True) & Q(is_superuser=False))
            except MyUser.DoesNotExist:
                raise ErrorMessage("QA ID is invalid")
            user_obj.is_active = False
            user_obj.save()
            return Response({"data": {"is_user_deleted": True, 'message': 'QA Profile Deleted successfully'}},
                            status=status.HTTP_200_OK)
        except ErrorMessage as e:
...
class QaAgentDetail(APIView):
    ...
    def get(self, request):
        try:
            agent_id = request.query_params.get('agent_id')
            try:
                user_obj = MyUser.objects.get(Q(agent_id=agent_id) & Q(is_qa_agent=True))
            except MyUser.DoesNotExist:
                raise ErrorMessage("QA ID is Invalid")
            serializer = QaUserListingSerializer(user_obj, many=False)
