...
from app.serializers import ProductListingSerializer, OrderListingSerializer, OrderDetailSerializer, \
    WorkOrderListingSerializer, MarkedOrderListingSerializer, AgentOrderDetailSerializer
from app.utilities import ErrorMessage, pagination, productsearch, ordersearch, orderfilter, orderdetailfilter, \
    ...
    orderfilter1, orderfilter3, orderfilter4
...
from app.permissions import SalesAgentValidation, AdminValidation, QaAdminValidation, QaAgentValidation, \
    AgentAdminValidation


class MarkedOrderlisting(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if from_date and end_date:
            ...
            from_date = None
            end_date = None
        queryset = SalesOrder.objects.all().filter(ordersearch(search) & orderfilter4(self.request) & Q(is_draft=False)
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if count:
                serializer = MarkedOrderListingSerializer(data_obj, many=True)
...
class AgentOrderDetail(APIView):
    permission_classes = (IsAuthenticated, AgentAdminValidation)
    ...
    def get(self, request):
        try:
            ...
            try:
                order_obj = SalesOrder.objects.get(Q(id=order_id) & orderdetailfilter(request) & Q(is_draft=False))
...
class AgentOrderStatus(APIView):
    permission_classes = (IsAuthenticated, AgentAdminValidation)

    def post(self, request):
        try:
            order_status = ['resolved', 'not_resolved']
            order_id = int(request.data['order_id'])
            try:
                sales_obj = SalesOrder.objects.get(Q(id=order_id) & Q(is_draft=False) & orderfilter3(request))
            except SalesOrder.DoesNotExist:
                raise ErrorMessage("Order is already Resolved or it's Invalid")
            try:
                customer_obj = Customer.objects.get(id=sales_obj.customer.id)
            except Customer.DoesNotExist:
                raise ErrorMessage("Customer ID is Invalid")
            agent_updated_status = request.data['order_status']
            if agent_updated_status not in order_status:
                raise ErrorMessage(f"Order Status must be {order_status} values only")

            agent_reason = str(request.data.get('reason', '')).strip()
            sales_obj.agent_updated_status = agent_updated_status
            sales_obj.agent_reason = agent_reason
            sales_obj.agent_status_updated_timestamp = datetime.datetime.now()
            sales_obj.save()
            data_obj = AgentQaReasons.objects.create(order=sales_obj, agent=request.user, orderstatus=agent_updated_status, reason=agent_reason)
